...
export function Header() {
  ...
  const phaseDisplay: Record<string, string> = {
    ...
    action_execution: 'Configure Actions',
  ...
  return (
    <header className="relative bg-slate-950/60 backdrop-blur-sm border-b border-gravity-border/20">
      ...
      <div className="min-h-11 px-3 py-2 sm:px-4 flex flex-wrap items-center gap-x-5 gap-y-2">
        {/* Logo and title */}
        <div className="flex items-center gap-3">
          {/* Mini gravity well icon */}
          <svg width="24" height="24" viewBox="0 0 24 24" className="text-blue-400" aria-hidden="true">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold uppercase tracking-wide bg-slate-800/80 border border-blue-500/40 text-blue-100 shadow-[0_0_10px_rgba(59,130,246,0.25)]">
              Early Beta
            </span>
          </div>
        </div>
        ...
        {/* Turn and phase info */}
        <div className="flex items-center gap-3 sm:gap-5 ml-auto xl:ml-0">
          <div className="flex items-center gap-2">
            <span className="text-[10px] uppercase tracking-widest text-gravity-muted">Turn</span>
            <span data-testid="current-turn" className="font-display text-lg font-bold tabular-nums text-slate-100">{game.currentTurn}</span>
          </div>
        ...
        {/* Controls */}
        <div className="w-full xl:w-auto xl:ml-auto flex flex-wrap items-center gap-1.5">
          <label className="text-[10px] uppercase tracking-widest text-gravity-muted flex items-center gap-1.5 mr-1">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="opacity-50" aria-hidden="true">
          ...
          <button
            ...
            title={botPlayerEnabled ? 'Opponent ships are controlled by AI' : 'Opponent ships will skip their actions'}
            aria-pressed={botPlayerEnabled}
          >
            {botPlayerEnabled ? 'Opponent AI: ON' : 'Opponent AI: OFF'}
          </button>
          <button
            type="button"
            onClick={newGame}
